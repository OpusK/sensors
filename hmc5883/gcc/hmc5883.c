...
uint8_t hmc5883CheckDeviceID(hmc5883_t *p_hmc5883);
...
uint8_t hmc5883CheckDeviceID(hmc5883_t *p_hmc5883)
{
  ...
  if ((hmc5883Read8(p, HMC5883_REG_IDENT_A) != 0x48)
      || (hmc5883Read8(p, HMC5883_REG_IDENT_B) != 0x34)
      || (hmc5883Read8(p, HMC5883_REG_IDENT_C) != 0x33))
  {
    return false;
  }
...
static uint8_t hmc5883Read8(hmc5883_t *p_hmc5883, uint8_t reg)
{
  ...
  i2cReadByte(p_hmc5883->ch, HMC5883_ADDRESS, reg, &data);
...
static int16_t hmc5883Read16(hmc5883_t *p_hmc5883, uint8_t reg)
{
  ...
  i2cReadBytes(p_hmc5883->ch, HMC5883_ADDRESS, reg, data, 2);
...
static void hmc5883Write8(hmc5883_t *p_hmc5883, uint8_t reg, uint8_t data)
{
  i2cWriteBytes(p_hmc5883->ch, HMC5883_ADDRESS, reg, &data, 1);
}
...
int hmc5883Cmdif(int argc, char **argv)
{
  ...
  if (ret == false)
  {
    cmdifPrintf("hmc5883 ch[1~9] read\n");
  }

  return 0;
}

#endif /* _USE_HW_CMDIF_HMC5883 */

#endif /* _USE_HW_HMC5883 */
